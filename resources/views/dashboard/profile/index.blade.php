...
<div class="container" style="max-width: 600px;">
    <div class="form-card" style="margin-top: 2rem;">
        @if(session('success'))
            <div class="alert alert-success" style="color: #fff; background: #4caf50; padding: 10px 20px; border-radius: 6px; margin-bottom: 18px;">
                {{ session('success') }}
            </div>
        @endif
        ...
        <div style="text-align:center; margin-bottom:24px;">
            <div class="avatar"
                ...
                @else
                    <span style="font-size:3em;line-height:110px; color:#6366f1;">{{ strtoupper(substr($user->name,0,1)) }}</span>
                @endif
            </div>
            ...
            <div style="font-size:0.95em; color:#888; margin-top:6px;">Haz clic en la imagen para cambiar tu foto de perfil</div>
        </div>
        ...
        <form action="{{ route('admin.profile.update') }}" method="POST" style="margin-top: 1.5rem;">
            ...
            <table class="table-profile" style="width:100%;">
                <tr>
                    ...
                    <td>
                        <input type="text" name="name" value="{{ old('name', $user->name) }}" required class="form-control" style="width:100%;">
                ...
                <tr>
                    <th>Email</th>
                    <td>
                        <input type="email" name="email" value="{{ old('email', $user->email) }}" required class="form-control" style="width:100%;">
...
cropButton.addEventListener('click', function () {
    ...
    cropper.getCroppedCanvas({
        ...
        .then(data => {
            if (data.success) {
                document.querySelector('.user-avatar').src = data.image_path + '?t=' + new Date().getTime();
                document.getElementById('image-upload-msg').innerText = 'Imagen actualizada correctamente';
                closeCropPopup();
            } else {
                document.getElementById('image-upload-msg').innerText = 'Error: ' + data.error;
            }
        })
        .catch(() => {
            document.getElementById('image-upload-msg').innerText = 'Error al subir la imagen';
        });
    }, 'image/png');
});
